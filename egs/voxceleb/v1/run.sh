...
#nnetdir=$exp/xvector_nnet_tdnn_softmax_1e-2_g2
#nnet/run_train_nnet.sh --cmd "$cuda_cmd" --env tf_gpu --continue-training false nnet_conf/tdnn_softmax_1e-2_g2.json \
#    $data/voxceleb_train_combined_no_sil/train $data/voxceleb_train_combined_no_sil/train/spklist \
#    $data/voxceleb_train_combined_no_sil/valid $data/voxceleb_train_combined_no_sil/train/spklist \
#    $nnetdir
...
#nnetdir=$exp/xvector_nnet_tdnn_amsoftmax_m0.20_linear_bn_1e-2_g2
#nnet/run_train_nnet.sh --cmd "$cuda_cmd" --env tf_gpu --continue-training false nnet_conf/tdnn_amsoftmax_m0.20_linear_bn_1e-2_g2.json \
#    $data/voxceleb_train_combined_no_sil/train $data/voxceleb_train_combined_no_sil/train/spklist \
#    $data/voxceleb_train_combined_no_sil/valid $data/voxceleb_train_combined_no_sil/train/spklist \
#    $nnetdir
...
#    $nnetdir
  ...
  pretrain_nnet=$exp/xvector_nnet_tdnn_softmax_1e-2_g2
  nnetdir=$exp/xvector_nnet_tdnn_softmax_1e-2_tdnn4_att_pretrain
  nnet/run_finetune_nnet.sh --cmd "$cuda_cmd" --env tf_gpu --continue-training false --checkpoint 'last' \
    nnet_conf/tdnn_softmax_1e-2_tdnn4_att_pretrain.json \
    $data/voxceleb_train_combined_no_sil/train $data/voxceleb_train_combined_no_sil/train/spklist \
    $data/voxceleb_train_combined_no_sil/valid $data/voxceleb_train_combined_no_sil/train/spklist \
...
if [ $stage -le 12 ]; then
  ...
  paste -d ' ' $voxceleb1_trials $nnetdir/scores/scores_voxceleb_test.plda | grep ' nontarget ' | awk '{print $NF}' > $nnetdir/scores/scores_voxceleb_test.plda.nontarge
...
if [ $stage -le 15 ]; then
  # Cosine similarity
  mkdir -p $nnetdir/scores_cos
  cat $voxceleb1_trials | awk '{print $1, $2}' | \
    ivector-compute-dot-products - \
      "ark:ivector-normalize-length scp:$nnetdir/xvectors_voxceleb_test_cos/xvector.scp ark:- |" \
      "ark:ivector-normalize-length scp:$nnetdir/xvectors_voxceleb_test_cos/xvector.scp ark:- |" \
      $nnetdir/scores_cos/scores_voxceleb_test.cos

  eer=`compute-eer <(local/prepare_for_eer.py $voxceleb1_trials $nnetdir/scores_cos/scores_voxceleb_test.cos) 2> /dev/null`
  mindcf1=`sid/compute_min_dcf.py --c-miss 10 --p-target 0.01 $nnetdir/scores_cos/scores_voxceleb_test.cos $voxceleb1_trials 2> /dev/null`
  mindcf2=`sid/compute_min_dcf.py --p-target 0.001 $nnetdir/scores_cos/scores_voxceleb_test.cos $voxceleb1_trials 2> /dev/null`
  echo "EER: $eer%"
  echo "minDCF(p-target=0.01): $mindcf1"
  echo "minDCF(p-target=0.001): $mindcf2"

  # Comment the following lines if you do not have matlab.
  paste -d ' ' $voxceleb1_trials $nnetdir/scores_cos/scores_voxceleb_test.cos | grep ' target ' | awk '{print $NF}' > $nnetdir/scores_cos/scores_voxceleb_test.cos.target
  paste -d ' ' $voxceleb1_trials $nnetdir/scores_cos/scores_voxceleb_test.cos | grep ' nontarget ' | awk '{print $NF}' > $nnetdir/scores_cos/scores_voxceleb_test.cos.nontarget
  comm=`echo "addpath('../../../misc/DETware_v2.1');Get_DCF('$nnetdir/scores_cos/scores_voxceleb_test.cos.target', '$nnetdir/scores_cos/scores_voxceleb_test.cos.nontarget', '$nnetdir/scores_cos/scores_voxceleb_test.cos.result');"`
  echo "$comm"| matlab -nodesktop > /dev/null
  tail -n 1 $nnetdir/scores_cos/scores_voxceleb_test.cos.result
fi
